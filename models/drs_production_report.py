...
class MRpDrsProduction(models.Model):
    ...
    _sql_constraints = [
        ('check_positive_metrics',
         'CHECK(thickness >= 0 AND length >= 0 AND final_weight >= 0)',
         'Thickness, length, and final weight cannot be negative.'),
        ('unique_roll_per_machine_date',
         'UNIQUE(output_roll_number, machine_number, date)',
         'This Output Roll Number has already been logged for this machine on this date!')
    ]
    ...
    # Python Validation for Working Hours
    @api.constrains('time_from', 'time_to')
    def _check_valid_times(self):
        for rec in self:
            if rec.time_from < 0 or rec.time_from >= 24 or rec.time_to < 0 or rec.time_to >= 24:
                raise ValidationError(_("Working times must be a valid 24-hour format (between 0.00 and 23.99)."))
            if rec.time_from >= rec.time_to and rec.time_to != 0.0:
                raise ValidationError(_("Time To must be strictly later than Time From."))
    ...
    def _onchange_shift(self):
        """Automatically clear mismatched supervisors and technicians if the shift changes."""
        if self.shift:
            if self.supervisor_id and self.supervisor_id.drs_shift not in [self.shift, 'both']:
                self.supervisor_id = False

            if self.technician_ids:
                valid_techs = self.technician_ids.filtered(lambda t: t.drs_shift in [self.shift, 'both'])
                self.technician_ids = valid_techs
    ...
    def _generate_excel_action(self, records):
        if len(records) > 200:
            raise UserError(
                _("Server Limit Reached: You can only export up to 200 records at a time to prevent server timeouts. Please use the filters to narrow down your selection."))
        ...
        try:
            ...
            for rec in records:
                ...
                ws.merge_range('F4:J4',
                               f"السمك: {txt(rec.thickness, '......')}      الطول: {txt(rec.length, '......')}",
                               fmt_label_r)
                ws.merge_range('A4:D4', f"التصنيف: {txt(rec.category)}", fmt_label_r)

                ws.merge_range('M5:O5', f"الوجه: {txt(rec.face)}", fmt_label_r)
                ws.merge_range('A5:K5',
                               f"الوزن النهائي: {txt(rec.final_weight, '..........')}      (عند نهاية رش الوجهين)",
                               fmt_label_c)
                ...
                ws.merge_range('A6:D6', f"المشرف: {rec.supervisor_id.name if rec.supervisor_id else txt('')}",
                               fmt_label_r)
                ...
                for ext_a, ext_b in extruder_pairs:
                    ...
                    ws.merge_range(row_start, 1, row_start + 4, 1,
                                   val(line_a_main.extruder_speed) if line_a_main else '',
                                   fmt_cell)
                    ws.merge_range(row_start, 2, row_start + 4, 2, val(line_a_main.air_pressure) if line_a_main else '',
                                   fmt_cell)
                    ws.merge_range(row_start, 6, row_start + 4, 6,
                                   val(line_a_main.melt_flow_rate) if line_a_main else '',
                                   fmt_cell)

                    ws.merge_range(row_start, 8, row_start + 4, 8, f"Ext. {ext_b}", fmt_merged_ext)
                    line_b_main = lines_dict.get(ext_b, {}).get('zone5')
                    ws.merge_range(row_start, 9, row_start + 4, 9,
                                   val(line_b_main.extruder_speed) if line_b_main else '',
                                   fmt_cell)
                    ws.merge_range(row_start, 10, row_start + 4, 10,
                                   val(line_b_main.air_pressure) if line_b_main else '',
                                   fmt_cell)
                    ws.merge_range(row_start, 14, row_start + 4, 14,
                                   val(line_b_main.melt_flow_rate) if line_b_main else '',
                                   fmt_cell)
            ...
            raise UserError(
                _("An unexpected error occurred while compiling the Excel file. \n\nTechnical details: %s") % str(e))
